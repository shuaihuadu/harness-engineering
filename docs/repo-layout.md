...
本文件是 Harness Engineering 规范的目录与套件章节，与 [`../README.md`](../README.md) 主体并列。全文采用第 10 节的编号体系，其他文档可按 `第 10.1 节`、`第 10.2 节` 稳定引用。
...
### 10.3 项目级索引（dev-map）：让 AI 知道"从哪进门"

随着项目持续迭代，新一轮需求分析或开发 Agent 进场时最容易出的问题不是"这次需求理解错了"，而是"重新发明了一遍轮子 / 把已有方案冲掉了"。原因往往不在模型本身，而在它根本不知道这座工程已经长成了什么样。

本规范用**两个互补的入口**给项目装一份索引，避免让 AI 一上来读全仓百科：

| 入口                                              | 维护者                | 偏向 | 回答的问题                                                |
| ------------------------------------------------- | --------------------- | ---- | --------------------------------------------------------- |
| 本文件（`docs/repo-layout.md`） + 顶层 `AGENTS.md` | RepoImpactMapper / 人 | 代码 | 某类功能落在哪、影响哪、惯例是什么；从哪个 README 进门    |
| [`templates/task-board.md`](../templates/task-board.md)（采用方仓库运行时位于 `docs/06-tasks/task-board.md`） | 调度者 / PM            | 需求 | 当前在做什么、做到哪一步、对应文档落在哪里、有没有重叠的旧需求 |
