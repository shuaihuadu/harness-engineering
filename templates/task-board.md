...
> 阶段定义见 `{{VENDOR_DIR}}/docs/stages/`，仓库结构入口见 `{{VENDOR_DIR}}/docs/repo-layout.md`。
...
字段约定：
- **当前阶段**：取自 H1–H6（阶段定义见 `{{VENDOR_DIR}}/docs/stages/`）；阶段切换时更新本字段，旧阶段的产物路径在文档目录字段保留可点
...
- 代码侧的"从哪进门"：见 `{{VENDOR_DIR}}/docs/repo-layout.md`（仓库结构、目录职责、模块落点）
- 阶段定义与门禁：见 `{{VENDOR_DIR}}/docs/stages/`
