...
字段约定：
- **当前阶段**：取自 H1–H6，见 [`docs/stages.md`](../docs/stages.md)；阶段切换时更新本字段，旧阶段的产物路径在文档目录字段保留可点
...
> 对应 Agent 阻塞返回（[`agents/_shared/io-contracts.md` 第 5 节](../agents/_shared/io-contracts.md)）的人工出口：阻塞条目应在这里有一行登记，避免只停在某个会话里。
...
- 代码侧的"从哪进门"：见 [`docs/repo-layout.md`](../docs/repo-layout.md)（仓库结构、目录职责、模块落点）
- 阶段定义与门禁：见 [`docs/stages.md`](../docs/stages.md)
