...
- 验证 `docs/04-detailed-design/` 下至少有 [`docs/stages/h3-detailed-design.md`](../../docs/stages/h3-detailed-design.md)列出的核心章节文件

任一不满足，按 [io-contracts.md 第 5 节](../_shared/io-contracts.md) 阻塞返回。

### 第二步：完备性扫描

按 [`docs/stages/h3-detailed-design.md`](../../docs/stages/h3-detailed-design.md)列出的章节逐项检查 `docs/04-detailed-design/` 下对应文件：
...
写入 `docs/04-detailed-design/design-review-report.md`（覆盖更新）。frontmatter 必填，`upstream` 列出涉及的 REQ / ADR。报告结构按 [`AGENT.md` 第 4 节](AGENT.md)。
...
- 设计目录严重偏离 [`docs/stages/h3-detailed-design.md`](../../docs/stages/h3-detailed-design.md)阻塞返回时给出明确的 `suggested_next_action`，不要尝试用部分数据写"半个报告"。
