...
6. **能选就别让填**：凡是封闭枚举（如"页面状态四态 / 加载形式 spinner-vs-skeleton / 权限差异类型"）、半结构化（如"决策人 / 决策日期"）、或预枚举的 OQ 候选答 A/B/C，**必须**按 [io-contracts.md §6.1](../_shared/io-contracts.md#61-交互式输入约定pick-over-type) 用 `ask.user` picker；自由 prose（页面布局描述）保持 chat 文本反问。
