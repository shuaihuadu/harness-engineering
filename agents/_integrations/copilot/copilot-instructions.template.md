...
本仓库采用 [Harness Engineering 规范]({{HARNESS_REPO_REF_FROM_GITHUB}}/HANDBOOK.md) 作为 AI 协作的工程骨架。下方规则面向 GitHub Copilot，按"目录"原则编写：只列硬约束，详细规则全部指向源文档。
...
当前项目已装的 Custom Agent（在 VS Code Copilot Chat 输入框下方的 Agent 下拉菜单中选择）。Agent 名以 `h<阶段号>-` 开头，对应 [Harness 阶段]({{HARNESS_REPO_REF_FROM_GITHUB}}/docs/stages/) H1–H6，横切阶段用 `hx-`：
...
| H3 协作起草详细设计（per-module）| `h3-design-author`            |
...
- 操作手册（10 分钟上手）：[`HANDBOOK.md`]({{HARNESS_REPO_REF_FROM_GITHUB}}/HANDBOOK.md)
- 阶段细则：[`docs/stages/`](../../../docs/stages/README.md)（H1–H6）
- 目录规范：[`docs/repo-layout.md`]({{HARNESS_REPO_REF_FROM_GITHUB}}/docs/repo-layout.md)
- 技术债务 GC：[`docs/tech-debt-gc.md`]({{HARNESS_REPO_REF_FROM_GITHUB}}/docs/tech-debt-gc.md)
...
五条："杂烩会话 / 反复纠错 / 过量规则文件 / 先信后验缺口 / 无界探索"。当用户请求触发任一条时，先指出反模式，再给替代做法。详见 [`{{VENDOR_DIR}}/docs/ai-usage.md` §4](../{{VENDOR_DIR}}/docs/ai-usage.md)。
