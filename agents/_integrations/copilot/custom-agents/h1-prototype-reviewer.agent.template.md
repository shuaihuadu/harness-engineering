...
> **工具集设计说明**：本 Agent 与 `/run-gate` 同属"只读评审员"角色，工具集刻意限制为 `search/*` + `read/*`——比 `/run-gate` 多一个 `read/viewImage`（用来读 `prototypes/<feature>/screenshots/` 下的截图）。**没有任何 `edit/*` / `execute/*` / `web/*` / `browser/*`**：评审员不写文件（评审纪要由人写）、不跑命令、不开浏览器抓页面。原型的渲染与自截屏交给上游 `H1-PrototypeAuthor` 负责，本 Agent 只消费他留下的 markdown 描述与截图；让评审 Agent 亲自重新点页面 / 做交互误差比对，是 v2 的事。
