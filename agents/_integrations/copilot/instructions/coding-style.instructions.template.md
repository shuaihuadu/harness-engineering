...
本规则覆盖 `src/**` 下的全部源码。以下要求与 Harness Engineering 规范 §6（H3 详细设计）和 [`docs/stages.md`]({{HARNESS_REPO_REF_FROM_GITHUB}}/docs/stages.md) §8（H5 编码与自验证）一致，AI 写代码时必须遵守。
...
- 提交前必须运行 `{{TEST_COMMAND}}` 并贴出真实输出
- 测试失败时分析原因再改代码，不要靠"再试一次"通过
- 风格检查：`{{LINT_COMMAND}}`，警告视作错误处理

## 4. 反模式（出现即停止）

照搬 [Harness Engineering 规范 §6.4]({{HARNESS_REPO_REF_FROM_GITHUB}}/README.md) 的五条：
