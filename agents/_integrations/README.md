...
> Copilot 一栏指向子目录而非单个文件——它包含一份顶层指令（`copilot-instructions.template.md`）、3 份切片指令（`instructions/`）、3 份专用 Custom Agent（`custom-agents/`），以及一份通用 Custom Agent 模板 `custom-agent.md.template`（可派生其他 5 个 Agent）。详细文件清单与复制步骤见 [copilot/README.md](./copilot/README.md)。
...
| 占位符                          | 含义                                                                                                                                                |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `{{PROJECT_NAME}}`              | 采用方仓库名 / 项目名                                                                                                                               |
| `{{PROJECT_ONE_LINER}}`         | 项目一句话简介                                                                                                                                      |
| `{{PRIMARY_LANGUAGE}}`          | 主语言（如 `C#`、`TypeScript`）                                                                                                                     |
| `{{TECH_STACK}}`                | 技术栈关键字                                                                                                                                        |
