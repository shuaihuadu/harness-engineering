...
3. **对每种语言**：检测 `.github/instructions/<lang>.instructions.md` 是否存在
   ...
   - **不存在** → 用 `ask.user` picker（参考 [io-contracts.md §6.1](../_shared/io-contracts.md#61-交互式输入约定pick-over-type)）问用户处置，**无 default、无 recommended**：
...
- **禁止**：
  - 修改非"允许修改的文件"清单中的文件
  - 修改 `{{VENDOR_DIR}}/` 目录下的任何规范 / Agent 文件
